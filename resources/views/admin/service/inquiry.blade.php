    ...
    <div class="container-fluid flex-grow-1 container-p-y">
        ...
        <div class="card ">
            <div class="card-header d-flex justify-content-between align-items-center border-bottom-1">
                <h5 class="mb-0">Service Inquiries</h5>
    ...
    <div class="modal-content">
      ...
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title">
          <i class="fa-solid fa-circle-info me-2"></i>Service Inquiry Details
        </h5>
...
$(document).ready(function() {
    $('#rentInquiryTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.services.inquiry') }}",
        columns: [
...
$(document).ready(function() {
    ...
    $(document).on('click', '.viewInquiryBtn', function() {
        ...
        $.ajax({
            url: "{{ route('admin.services.inquiry.show') }}", // create this route
            type: "GET",
            data: { id: inquiryId },
    ...
    $(document).on('click', '#markRepliedBtn', function() {
        let id = $(this).data('id');
        $.ajax({
            url: "{{ route('admin.services.inquiry.updateStatus') }}",
            type: "POST",
            data: {
                id: id,
                _token: "{{ csrf_token() }}"
